...
import { getMatchingMessageKey } from '@/lib/textMatching';
...
export function MatchingResultDisplay({ result }: MatchingResultDisplayProps) {
  const t = useTranslations('matchingResult');
  const tf = useTranslations('feedback');
  const messageKey = getMatchingMessageKey(result.similarity);
  const message = tf(messageKey);
