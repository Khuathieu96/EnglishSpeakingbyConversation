...
export function BrowserWarning() {
  ...
  useEffect(() => {
    ...
    const isFullySupported =
      hasSpeechRecognitionApi && hasSpeechSynthesis && hasMediaRecorder;
      ...
      <div className='flex'>
        ...
        <div className='ml-3'>
          ...
          <div className='mt-2 text-sm text-yellow-700'>
            <p>{t('recommendation', { browserName })}</p>
            {browserName === 'Safari' && (
              <p className='mt-2'>{t('safariWarning')}</p>
            )}
            {browserName === 'Firefox' && (
              <p className='mt-2'>{t('firefoxWarning')}</p>
            )}
          </div>
        </div>
      </div>
